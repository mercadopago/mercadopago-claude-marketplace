...
- Deterministic integration contracts and smoke coverage for Bricks, Point, QR,
...
- Public CI now uses Node 20, installs dependencies from the public npm registry,
  audits npm dependencies, runs every deterministic product suite, validates the
  plugin strictly, and rejects catalog drift.
...
- The smoke lockfile no longer references Mercado Libre's internal npm registry,
  and public documentation no longer contains maintainer-specific absolute paths.
